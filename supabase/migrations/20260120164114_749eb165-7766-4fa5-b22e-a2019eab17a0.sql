...
BEGIN
  ...
  ELSIF NEW.approval_status = 'rejected' AND (OLD.approval_status IS NULL OR OLD.approval_status != 'rejected') THEN
    ...
    VALUES (
      NEW.rep_id,
      'goal_rejected',
      'Goal Requires Changes',
      'Your goal "' || NEW.title || '" needs revisions: ' || COALESCE(NEW.rejection_reason, 'No reason provided'),
      '/bd/accountability/' || NEW.id,
      jsonb_build_object(
        'goal_id', NEW.id,
        'goal_title', NEW.title,
...
CREATE TRIGGER trigger_notify_goal_submitted
  AFTER INSERT OR UPDATE ON public.accountability_rep_goals
  FOR EACH ROW
