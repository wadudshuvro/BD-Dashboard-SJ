...
const VideoPage = () => {
  ...
  const {
    ...
    refetchInterval: (data) => {
      ...
      const hasActive = data.some((video) => isVideoProcessingStatus(video.status));
  ...
  const sortedVideos = useMemo(() => {
    if (!videos.length) return [] as SoraVideo[];
            ...
            <div className="space-y-4">
              <DialogHeader>
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant="secondary">{getVideoStatusLabel(selectedVideo.status)}</Badge>
              ...
              {selectedVideo.prompt && (
                <div className="rounded-lg border border-border/60 bg-muted/40 p-4">
                  ...
                  <p className="mt-1 text-sm leading-relaxed text-foreground">{selectedVideo.prompt}</p>
                </div>
              )}
            </div>
          ) : null}
