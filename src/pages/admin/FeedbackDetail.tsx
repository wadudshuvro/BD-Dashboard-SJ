...
export default function AdminFeedbackDetail() {
  ...
  const priorityMutation = useMutation({
    ...
    onSuccess: (updated) => {
      toast({
        ...
        description: updated.priority ? `Priority set to ${updated.priority}.` : "Priority cleared.",
      });
      queryClient.invalidateQueries({ queryKey: ["feedback-detail", id] });
      queryClient.invalidateQueries({ queryKey: ["feedback-list"] });
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: "The item was marked as closed.",
      });
      queryClient.invalidateQueries({ queryKey: ["feedback-list"] });
      navigate("/adminpanel/feedback");
    },
    onError: (error) => {
      toast({
  ...
  if (!detailQuery.data?.feedback) {
    return (
      <div className="py-12 text-center">
        ...
        <Button variant="outline" onClick={() => navigate("/adminpanel/feedback")}>
          <ArrowLeft className="h-4 w-4 mr-2" /> Back to Feedback Manager
        </Button>
      </div>
    );
  }
  ...
  const isUpdating =
    statusMutation.isPending || priorityMutation.isPending || deleteMutation.isPending;

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => navigate("/adminpanel/feedback")}>
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <FeedbackBreadcrumb
          parentLabel="Feedback Manager"
          parentPath="/adminpanel/feedback"
